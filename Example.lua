...
local repo = 'https://raw.githubusercontent.com/zulquix/custom-ui2/main/'
...
-- New: Toggle-mode keybind with chord modifiers
LeftGroupBox:AddLabel('Keybind (Toggle + modifiers)'):AddKeyPicker('ToggleKeybind', {
    Default = 'K',
    Mode = 'Toggle',
    Modifiers = { Ctrl = true, Shift = false, Alt = false },
    Text = 'Ctrl+K toggle keybind',
    Callback = function(Value)
        Library:Notify('Ctrl+K toggled: ' .. tostring(Value), 2)
    end,
})

-- New: Hold-mode keybind with chord modifiers
LeftGroupBox:AddLabel('Keybind (Hold + modifiers)'):AddKeyPicker('HoldKeybind', {
    Default = 'H',
    Mode = 'Hold',
    Modifiers = { Ctrl = false, Shift = true, Alt = false },
    Text = 'Shift+H hold keybind',
})
...
task.spawn(function()
    while true do
        ...
        if Options.HoldKeybind and Options.HoldKeybind:GetState() then
            print('HoldKeybind is being held')
        end
    end
end)
...
LeftGroupBox:AddButton({
    ...
    Func = function()
        Library:Notify('Default notification', 2)
    ...
    Tooltip = 'Demonstrates stacking + animations'
})
...
RightGroupbox:AddToggle('DependentToggle', {
    ...
    Tooltip = 'This toggle only works when ControlToggle is ON',
    Callback = function(v)
        Library:Notify('DependentToggle changed: ' .. tostring(v), 2)
    end,
})

-- New: Cooldown toggle
RightGroupbox:AddToggle('CooldownToggle', {
    Text = 'Cooldown toggle (1.5s)',
    Default = false,
    Cooldown = 1.5,
    Tooltip = 'Spam protected using Cooldown option',
    Callback = function(v)
        Library:NotifyWarning('CooldownToggle: ' .. tostring(v), 2)
    end,
})
