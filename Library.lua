...
function Library:FadeIn(inst, duration)
    if inst:IsA('GuiObject') then
        ...
        self:Tween(inst, ti, { BackgroundTransparency = math.clamp(inst.BackgroundTransparency, 0, 1) })
    end
end

function Library:SmoothProperty(inst, prop, value, duration)
    local ti = TweenInfo.new(duration or 0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    return self:Tween(inst, ti, { [prop] = value })
end

function Library:RegisterCustomTheme(name, themeTable)
    assert(type(name) == 'string' and #name > 0, 'RegisterCustomTheme: name must be a string')
    assert(type(themeTable) == 'table', 'RegisterCustomTheme: themeTable must be a table')

    local base = self.Themes.Default
    local t = {}
    for k, v in next, base do
        t[k] = themeTable[k] or v
    end
    self.Themes[name] = t
end
...
function Library:SetTheme(name)
    ...
    self.FontColor = theme.FontColor
    self.MainColor = theme.MainColor
    self.BackgroundColor = theme.BackgroundColor
    self.AccentColor = theme.AccentColor
    self.OutlineColor = theme.OutlineColor
    self.RiskColor = theme.RiskColor
...
function Library:ToggleRGB(enabled, speed)
    self.RGB.Enabled = not not enabled
    if type(speed) == 'number' and speed > 0 then
        self.RGB.Speed = speed
    end
end
...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        local function getModifierState()
            local ctrl = InputService:IsKeyDown(Enum.KeyCode.LeftControl) or InputService:IsKeyDown(Enum.KeyCode.RightControl)
            local shift = InputService:IsKeyDown(Enum.KeyCode.LeftShift) or InputService:IsKeyDown(Enum.KeyCode.RightShift)
            local alt = InputService:IsKeyDown(Enum.KeyCode.LeftAlt) or InputService:IsKeyDown(Enum.KeyCode.RightAlt)
            return ctrl, shift, alt
        end

        local function formatCombo(key, mods)
            if type(key) ~= 'string' then
                key = tostring(key)
            end
            mods = mods or KeyPicker.Modifiers
            local parts = {}
            if mods and mods.Ctrl then table.insert(parts, 'Ctrl') end
            if mods and mods.Shift then table.insert(parts, 'Shift') end
            if mods and mods.Alt then table.insert(parts, 'Alt') end
            table.insert(parts, key)
            return table.concat(parts, '+')
        end

        local function comboMatchesInput(input)
            local key = KeyPicker.Value
            if key == 'None' then return false end
            local ctrl, shift, alt = getModifierState()
            local mods = KeyPicker.Modifiers or {}
            if (mods.Ctrl and not ctrl) or (mods.Shift and not shift) or (mods.Alt and not alt) then
                return false
            end
        ...
        function KeyPicker:Update()
            ...
            ContainerLabel.Text = string.format('[%s] %s (%s)', formatCombo(KeyPicker.Value, KeyPicker.Modifiers), Info.Text, KeyPicker.Mode);
        ...
        function KeyPicker:SetValue(Data)
            if type(Data) == 'string' then
                ...
                DisplayLabel.Text = formatCombo(KeyPicker.Value, KeyPicker.Modifiers)
                KeyPicker:Update()
                return
            end
        ...
        PickOuter.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
                ...
                Event = InputService.InputBegan:Connect(function(Input)
                    ...
                    DisplayLabel.Text = formatCombo(Key, KeyPicker.Modifiers);
                    KeyPicker.Value = Key;
        ...
        Library:GiveSignal(InputService.InputBegan:Connect(function(Input)
            if (not Picking) then
                if KeyPicker.Mode == 'Toggle' then
                    if comboMatchesInput(Input) then
...
do
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        Toggle.Debounce = (Info.Debounce == nil) and true or (not not Info.Debounce)
        Toggle.Cooldown = (type(Info.Cooldown) == 'number' and Info.Cooldown > 0) and Info.Cooldown or 0
        Toggle.Dependencies = Info.Dependencies -- optional shorthand: { {Toggles.X, true}, ... }
        Toggle.DependencyMode = (Info.DependencyMode == 'Any' and 'Any') or 'All'
        Toggle._busy = false
        ...
        function Toggle:_dependenciesMet()
            if type(Toggle.Dependencies) ~= 'table' or #Toggle.Dependencies == 0 then
                return true
            end

            local any = false
            local all = true
            for i = 1, #Toggle.Dependencies do
                local dep = Toggle.Dependencies[i]
                local obj, want = dep[1], dep[2]
                local ok = (obj and obj.Value == want)
                any = any or ok
                all = all and ok
            end

            if Toggle.DependencyMode == 'Any' then
                return any
            end
            return all
        end
        ...
        function Toggle:SetValue(Bool)
            ...
            if Toggle._busy and Toggle.Debounce then
                return
            end

            if Toggle.Cooldown > 0 then
                local now = os.clock()
                local last = Library._cooldowns[Idx] or 0
                if (now - last) < Toggle.Cooldown then
                    return
                end
                Library._cooldowns[Idx] = now
            end

            if not Toggle:_dependenciesMet() then
                return
            end
...
function Library:_reflowNotifications()
...
function Library:ClearNotifications()
    for i = #self.NotificationQueue, 1, -1 do
        local n = table.remove(self.NotificationQueue, i)
        if n and n.Outer and n.Outer.Parent then
            pcall(function() n.Outer:Destroy() end)
        end
    end
end

function Library:_notifyColorForType(typ)
    if typ == 'Success' then
        return Color3.fromRGB(65, 200, 120)
    elseif typ == 'Warning' then
        return Color3.fromRGB(255, 200, 70)
    elseif typ == 'Error' then
        return self.RiskColor
    end
...
function Library:Notify(Text, Time, Type, Icon)
    ...
    local leftPadding = iconLabel and 24 or 4
    local notifyLabel = self:CreateLabel({
        Position = UDim2.new(0, leftPadding, 0, 0);
        Size = UDim2.new(1, -leftPadding, 1, 0);
        Text = Text;
        TextXAlignment = Enum.TextXAlignment.Left;
        TextSize = 14;
        ZIndex = 103;
    ...
    table.insert(self.NotificationQueue, 1, entry)

    while #self.NotificationQueue > (self.NotificationMax or 6) do
        local old = table.remove(self.NotificationQueue)
        if old and old.Outer and old.Outer.Parent then
            pcall(function() old.Outer:Destroy() end)
        end
    end
    ...
    task.spawn(function()
        ...
        wait(0.25)
        for i = #self.NotificationQueue, 1, -1 do
            if self.NotificationQueue[i] == entry then
...
function Library:NotifySuccess(Text, Time, Icon)
    return self:Notify(Text, Time, 'Success', Icon)
end

function Library:NotifyWarning(Text, Time, Icon)
    return self:Notify(Text, Time, 'Warning', Icon)
end

function Library:NotifyError(Text, Time, Icon)
    return self:Notify(Text, Time, 'Error', Icon)
end
...
function Library:CreateWindow(...)
    ...
    Library:ApplyGlassFrame(Inner, false, true, true, true)
    ...
    Library:ApplyModernText(WindowLabel, true, Library.Style.TextSizeTitle)
    ...
    Library:ApplyGlassFrame(MainSectionOuter, true, true, true, false)
    ...
    Library:ApplyGlassFrame(MainSectionInner, true, false, true, true)
    ...
    local TabListLayout = Library:Create('UIListLayout', {
        Padding = UDim.new(0, math.floor((Config.TabPadding or 0) * (Library.Style.SpacingScale or 0.7)));
        FillDirection = Enum.FillDirection.Horizontal;
        SortOrder = Enum.SortOrder.LayoutOrder;
        Parent = TabArea;
    });
    ...
    Library:ApplyGlassFrame(TabContainer, true, true, true, true)
    ...
    function Window:AddTab(Name)
        ...
        Library:ApplyModernText(TabButtonLabel, false, Library.Style.TextSizeBody)

        -- Subtle hover feedback for unselected tabs
        Library:ApplyMicroHover(TabButton, TabButton, function()
            local reg = Library.RegistryMap[TabButton]
            if reg and type(reg.Properties.BackgroundColor3) == 'function' then
                return reg.Properties.BackgroundColor3()
            end
            return TabButton.BackgroundColor3
        end)
        ...
        local sidePad = math.floor(8 * (Library.Style.SpacingScale or 0.7))
        local sideGap = math.floor(4 * (Library.Style.SpacingScale or 0.7))

        local LeftSide = Library:Create('ScrollingFrame', {
            BackgroundTransparency = 1;
            BorderSizePixel = 0;
            Position = UDim2.new(0, sidePad, 0, sidePad);
            Size = UDim2.new(0.5, -(sidePad + sideGap), 1, -(sidePad * 2));
            CanvasSize = UDim2.new(0, 0, 0, 0);
            BottomImage = '';
            TopImage = '';
        ...
        local RightSide = Library:Create('ScrollingFrame', {
            BackgroundTransparency = 1;
            BorderSizePixel = 0;
            Position = UDim2.new(0.5, sideGap, 0, sidePad);
            Size = UDim2.new(0.5, -(sidePad + sideGap), 1, -(sidePad * 2));
            CanvasSize = UDim2.new(0, 0, 0, 0);
            BottomImage = '';
            TopImage = '';
        ...
        Library:Create('UIListLayout', {
            Padding = UDim.new(0, math.floor(8 * (Library.Style.SpacingScale or 0.7)));
            FillDirection = Enum.FillDirection.Vertical;
            SortOrder = Enum.SortOrder.LayoutOrder;
        ...
        function Tab:ShowTab()
            ...
            Library:Tween(TabButton, Library.Animation.TweenInfoFast, { BackgroundColor3 = Library:GetDarkerColor(Library.AccentColor) })
            local reg = Library.RegistryMap[TabButton]
            if reg then
                reg.Properties.BackgroundColor3 = function()
        ...
        function Tab:AddGroupbox(Info)
            ...
            Library:ApplyGlassFrame(BoxOuter, true, true, true, false)
            ...
            Library:ApplyGlassFrame(BoxInner, true, false, true, true)
            ...
            Library:ApplyModernText(GroupboxLabel, true, 15)
            ...
            Library:Create('UIListLayout', {
                Padding = UDim.new(0, math.floor(4 * (Library.Style.SpacingScale or 0.7)));
                FillDirection = Enum.FillDirection.Vertical;
                SortOrder = Enum.SortOrder.LayoutOrder;
                Parent = Container;
            });
